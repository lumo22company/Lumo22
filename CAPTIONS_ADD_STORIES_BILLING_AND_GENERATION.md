...
So “add Stories at any time” means: **start a new checkout that includes Stories**. The app does **not** update an existing subscription to add the Stories price (no in-place “add another price to this subscription” flow).
...
- **Subscription:** A new subscription checkout with Stories creates a **new** Stripe subscription (base + Stories recurring). So if they already have a captions-only subscription and then do “add Stories” via `/captions?stories=1` and choose subscription, they would end up with **two** subscriptions (one captions-only, one captions+Stories) unless they cancel the first. The app does not:
  ...
  - Align the billing cycle when “adding” Stories.
...
| Billing impact | New purchase = new order; if subscription, that’s a new subscription (risk of two subscriptions if they already had one). No in-place add-to-existing-sub yet. |
...
1. **Stripe:** Use [Subscription update](https://stripe.com/docs/api/subscriptions/update) to add the Stories subscription price to the existing subscription (Stripe handles mid-cycle billing per its rules).
...
That would keep one subscription and one billing cycle, with Stories included from the next invoice (or with a partial-period charge, per Stripe).
