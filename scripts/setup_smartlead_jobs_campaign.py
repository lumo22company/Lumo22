#!/usr/bin/env python3
"""
Create Smartlead 'UK Hiring SMM — Job intent' campaign via API (no lead import).

Requires SMARTLEAD_API_KEY in .env (Smartlead → Settings → API).

Usage:
  python3 scripts/setup_smartlead_jobs_campaign.py
  python3 scripts/setup_smartlead_jobs_campaign.py --dry-run
"""
...
CAMPAIGN_NAME = "UK Hiring SMM — Job intent"
...
def main() -> None:
    ...
    ap.add_argument("--sophie-email-account-id", type=int, default=0, help="Smartlead email account ID for sophie@lumo22.com")
    args = ap.parse_args()
    ...
    print("Added 3-email sequence (v2)")
    ...
    try:
        ...
        print("Set UK schedule (5 leads/day)")
    ...
    out = ROOT / "exports" / "smartlead_jobs_campaign_created.json"
    out.write_text(
        json.dumps(
            {
                ...
                "status": "DRAFTED",
                "import_csv_when_ready": "exports/smartlead_new_only_jobs_smm_intent_employers_2026-06-18.csv",
                "leads_count": 10,
            },
            indent=2,
        ),
    ...
    print(f"Wrote {out}")
    print("\nNext: attach sophie@lumo22.com if needed, send test email, then import CSV when ready.")
