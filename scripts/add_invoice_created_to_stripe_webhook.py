#!/usr/bin/env python3
"""
Add invoice.created to Stripe webhook endpoint (for referrer reward: 10% off next billing period).
Run once: python scripts/add_invoice_created_to_stripe_webhook.py
"""
import os
...
if "invoice.created" in events:
    print(f"invoice.created already enabled on {target.url}")
    exit(0)

events.append("invoice.created")
stripe.WebhookEndpoint.modify(target.id, enabled_events=events)
print(f"Added invoice.created to {target.url}")
