...
**Deploy:** Railway — **default is `git push`** (e.g. to `main`) when the service is linked to GitHub; that triggers production. Use **`railway up --no-gitignore`** from the project root only when they want a CLI upload, push isn’t possible, or they ask for both (see `docs/RAILWAY_DEPLOY.md`).
