...
**Deploy / redeploy:** **Always use `git push`** (e.g. to `main`) when the service is linked to GitHub — that triggers production. Do **not** default to **`railway up`**. Use **`railway up --no-gitignore`** only when they explicitly want a CLI upload, push isn’t possible, or they ask for both (see `docs/RAILWAY_DEPLOY.md`).
